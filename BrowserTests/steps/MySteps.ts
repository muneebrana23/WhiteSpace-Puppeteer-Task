import { After, AfterAll, Before, BeforeAll, Given, setDefaultTimeout, Then } from "@cucumber/cucumber";
...
Given("I click the {string} button", async function(tabButton) {
  if(tabButton=="Learn More"){
    await page.waitForXPath("//a[text()='Learn more']", { visible: true},{timeout:5000}).then(button => button.click());
...
Then("the text {string} is displayed", async function(title) {
  ...
  if(title=="Introduction to the Angular Docs"){
...
Then("{string} found", async function(searchResult) {
 ...
 /* try {
    const xpathSelector="(//h3[@class='search-section-header'])[1]";
    await page.waitForXPath("(//h3[@class='search-section-header'])[1]");
    const [elementHandle] = await page.$x(xpathSelector);
    const text = await page.evaluate(element => element.textContent, elementHandle);
    console.log(text)
    expect(text).to.include(searchResult)
  } catch {
    console.log("Welcome Back")
    const [element] = await page.$x("//p[@class='no-results'][contains(text(),'No results found')]");
